...
def updateQueue (queue, days):
    tempq = []
    while len(queue) > 0:
        tempJob = heapq.heappop(qArrival)
        heapq.heappush(tempq, (tempJob[0] - days, tempJob[1], tempJob[2], tempJob[3]))
    return tempq
    ...
    while tdelta > 0:
        if current_date.weekday() < 4: #Mon-Thurs
            if (current_hour + tdelta) > 15*0.8: #means that it's more than one day
                ...
                qArrival = updateQueue(qArrival, 1)
        ...
        if current_date.weekday() == 4: #Fri
            if (current_hour + tdelta) > 7.5*0.8: #means that it's more than one day
                ...
                qArrival = updateQueue(qArrival, 3)
...
while True:
    ...
    for job in dictJobs:
        if dictJobs[job]["release"] <= current_date:
            if dictJobs[job]["status"] == "AWAITING":
                ...
                #Get EDD value
                difference = (dictJobs[job]["end"] - current_date).days
                heapq.heappush(qArrival, (difference, float(refer[1]), refer[0], job))
    ...
    while len(qArrival) > 0:
        ...
        if not dictMachines[tempJob[2]]:
            print "ASSIGNED: " + str(tempJob[3])
            # add to priority queue (job, + t=expected)
            heapq.heappush(qService, (t + tempJob[1], tempJob[2], tempJob[3]))
            dictJobs[tempJob[3]]["status"] = "SERVING"
            # Set machine index to true.
            dictMachines[tempJob[2]] = True
        else:
